...
const EventCard = ({ title, description, date, photos }) => {
    ...
    const maxLength = 280; // Adjust this value based on your needs
    const shouldShowReadMore = description.length > maxLength;

    const displayText =
        shouldShowReadMore && !isExpanded ? `${description.slice(0, maxLength)}...` : description;

    return (
        <div className="flex flex-col lg:flrex-row items-center rounded-xl overflow-hidden shadow-2xl px-3 pb-3 md:py-20 md:px-10 bg-gradient-to-b from-neutral-800 to-neutral-600">
