  ...
  return (
    <div className="max-w-sm bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 transform hover:scale-105">
        ...
        <div className="p-6">
            ...
            <p className=" max-w-72 mt-2 text-lg text-gray-500">{role}</p>
            {exrole && <p className=" max-w-72 mt-2 text-lg text-gray-500">{`Ex- ${exrole}`}</p>}
            {achieve && <p className="max-w-72 mt-2 text-lg text-gray-500">{`Achievements: ${achieve}`}</p>}
        </div>
    </div>
  )
}
